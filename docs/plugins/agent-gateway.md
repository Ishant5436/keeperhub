...
4. **Credential Storage Protection:** Configuration secrets (such as HMAC request secrets) are encrypted at rest with AES-256-GCM and stripped entirely from any connection-read response before it leaves the server. The provisioning endpoint returns the HMAC secret exactly once upon initial creation.
