...
4. **Credential Storage Protection:** Configuration secrets (such as HMAC request secrets) are encrypted at rest with AES-256-GCM and stripped entirely from any response before it leaves the server.
